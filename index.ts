...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		execute: async (_toolCallId, _params, _signal, _onUpdate, ctx) => {
			...
			if (!createdTasks && !createdRules) {
				return {
					content: [
						{ type: "text", text: `${TASKS_PATH} and ${RULES_PATH} already exist — read ${RULES_PATH} and follow it.` },
					],
			...
			return {
				content: [
					{ type: "text", text: parts.join(", ") + `. Read ${RULES_PATH} and follow it when recording work instructions.` },
				],
				details: { createdTasks, createdRules },
			};
		},
	...
	pi.registerCommand("tasks-init", {
		...
		handler: async (_args, ctx) => {
			...
			if (createdRules) ctx.ui.notify(`Created ${RULES_PATH} (recording rules).`, "info");
		},
	});
	...
	pi.registerCommand("tasks-info", {
		...
		handler: async (_args, ctx) => {
			ctx.ui.notify(
				`Usage: /tasks-init — generate ${TASKS_PATH}/${RULES_PATH} if missing · /tasks-tidy — normalize format (order-preserving) · /tasks-archive — move completed items to ${ARCHIVE_PATH} · /tasks-verify — walk pending confirmations with the user · /tasks-clear — clear ${TASKS_PATH} and write a tombstone note.`,
				"info",
			);
		},
	});
