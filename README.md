...
**The `tasks_tidy` tool.** The extension registers an agent-invocable tool so the model can tidy the file itself after checklist updates: same deterministic normalization as the command (checkbox syntax, indentation, `Confirm (user):` prefixes, unchecked items first), but without a dialog — the call and its result show up in the transcript. The standing rule tells the model to call it after updating the file. `tasks_tidy` is the only rewrite the agent can trigger on its own; it is non-destructive by construction (unrecognized lines pass through).
