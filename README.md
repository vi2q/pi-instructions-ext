...
**Parallel sessions.** The rules in `docs/RULES.md` define owner tags, and the session-start pointer provides the session's own tag (a short id hashed from the full pi session id, so every session gets its own, e.g. `(s3c9c)`): the model tags every instruction it records with it, updates only items carrying its own tag, and re-reads the file before writing so parallel sessions merge instead of overwriting. On top of that convention, the extension watches both files' content between turns: if `docs/TASKS.md` changed on disk since the agent last touched it (another session, or a manual edit), the per-turn reminder switches to a staleness warning telling the model to re-read and merge; if `docs/RULES.md` changed, it switches to a notice telling the model to re-read the rules, since the recording conventions may have changed. Either notice fires once, on the first turn after the change. The agent's own writes never trigger either.

**The `tasks_tidy` tool.** The extension registers an agent-invocable tool so the model can tidy the file itself after checklist updates: same deterministic normalization as the command (checkbox syntax, flat items, `Confirm (user):` prefixes), but without a dialog — the call and its result show up in the transcript. **Item order is always preserved** and item text is never rewritten; the one structural change is that nested checklist items are flattened to top level (checklist nesting is not a supported shape — see `docs/RULES.md`). The tool's own guidelines (appended to the system prompt) tell the model to call it after updating the file.
...
**`/tasks-archive`** moves every checked (`[x]`) item — at any nesting level — to a dated file under `docs/archives/` (`TASKS-YYYY-MM-DD.md`; same-day runs append a new `## Archived …` heading to the same file). A moved item keeps its notes and its sub-items; an unchecked item stays put and its checked descendants are extracted. Non-checkbox prose that is not indented under an item belongs to the section and stays in the file, so interleaved paragraphs are not swept into the archive. Archived items are written flat. A section heading is dropped only when its section becomes completely empty (a level-1 document title is always kept). A confirmation dialog shows the item count first.
