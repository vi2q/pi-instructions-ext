...
**The `tasks_tidy` tool.** The extension registers an agent-invocable tool so the model can tidy the file itself after checklist updates: same deterministic normalization as the command (checkbox syntax, indentation, `Confirm (user):` prefixes), but without a dialog — the call and its result show up in the transcript. **Item order is always preserved** — tidy never reorders, moves, or rewrites item text, so the model can call it without disturbing the document's narrative structure. The standing rule tells the model to call it after updating the file.
